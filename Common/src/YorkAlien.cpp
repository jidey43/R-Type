...
YorkAlien::YorkAlien(sf::Vector2f const& speed, sf::Vector2f const& pos, unsigned int id, float coeff)
...
bool	        YorkAlien::update(sf::Clock const& clock)
{
  ...
  this->_pos.y = _pos.y + _coeff * cos((0.01 * _f * (this->_pos.x / 150) * M_PI) + _rad);
